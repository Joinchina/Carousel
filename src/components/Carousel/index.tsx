            ...
            <div className="img_box" ref={demo}>
                {
                    imgUrl?.map((item,index)=>{
                        return (
                            <div style={{background:`url(${item.img}) bottom center no-repeat`,float:"left",width:`${clientWidth}px`,height:'600px',backgroundSize: 'auto 100%',backgroundColor:`${item.bgColor}`}} key={item.img}>
                                <h1 style={{color:`${item.color}`}}>{item.title}</h1>
                                <h2 style={{color:`${item.color}`}}>{item.describe}</h2>
                            </div>
                        )
                    })
                }
